...
export async function currentAdmin(request: Request, env: Env) {
  ...
  const row = await env.DB.prepare(
    "SELECT a.id,a.username,a.role,s.expires_at FROM admin_sessions s JOIN admins a ON a.id=s.admin_id WHERE s.token_hash=?"
  ).bind(await sha256(m[1])).first();
  if (!row || Number((row as any).expires_at) < Math.floor(Date.now() / 1000)) return null;
  return row;
}
