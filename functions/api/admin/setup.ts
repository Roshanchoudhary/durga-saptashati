...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    if (Number(count?.n || 0) > 0) return json({ error: "Admin already exists. Setup is closed." }, 403);
    ...
    if (!/^[A-Za-z0-9._-]{3,40}$/.test(username))
      return json({ error: "Username 3-40 characters: letters, numbers, . _ -" }, 400);
    ...
    ).bind(id, username, email || null, hash, salt, "admin").run();

    return json({ ok: true }, 201);
  } catch (e: any) {
    return json({ error: e?.message || "Setup failed" }, 500);
  }
};
