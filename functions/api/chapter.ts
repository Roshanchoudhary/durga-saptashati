...
export const onRequestGet:PagesFunction<Env>=async({request,env})=>{
 try{
  ...
  const x:any=await env.DB.prepare("SELECT * FROM chapters WHERE id=? AND status='published'").bind(id).first(); if(!x)return json({error:"Not found"},404);
...
export const onRequestDelete:PagesFunction<Env>=async({request,env})=>{
 try{
  if(!await currentAdmin(request,env))return json({error:"Unauthorized"},401);
  const id=new URL(request.url).searchParams.get("id"); if(!id)return json({error:"Missing id"},400);
  await env.DB.prepare("DELETE FROM chapters WHERE id=?").bind(id).run(); return json({ok:true});
